...
// TestOrganizationProfileAttr_TypeDefinition verifies OrganizationProfileAttr struct compiles with required fields.
func TestOrganizationProfileAttr_TypeDefinition(t *testing.T) {
	attr := profile.OrganizationProfileAttr{
		Repositories: []string{"repo1", "repo2"},
		Permissions:  []string{"contents:read", "packages:write"},
	}

	expected := profile.OrganizationProfileAttr{
		Repositories: []string{"repo1", "repo2"},
		Permissions:  []string{"contents:read", "packages:write"},
	}
...
// TestPipelineProfileAttr_TypeDefinition verifies PipelineProfileAttr struct compiles.
func TestPipelineProfileAttr_TypeDefinition(t *testing.T) {
	attr := profile.PipelineProfileAttr{}

	expected := profile.PipelineProfileAttr{}
	assert.Equal(t, expected, attr)
}
